...
impl Process {
    ...
    pub fn new_with_deadline(mut launch_options: LaunchOptions, deadline: Instant) -> Result<Self> {
    ...
    fn ws_url_from_output(child_process: &mut Child, timeout: Duration) -> Result<Url> {
        let chrome_output_result = util::Wait::with_timeout(timeout).until(|| {
